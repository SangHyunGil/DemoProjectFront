...
import {CategoryWrapper,Category,UnderLine} from '../Categories/Categories';
    ...
    return (
        <>
            <CategoryWrapper>
                ...
                key={cat.studyBoardId}>{cat.title}{IsSelected === cat.title ? <UnderLine variants={UnderLineVariants} initial="hidden" animate="visible" /> : null}</Category>))}
                {IsGranted && <Category onClick={()=>setIsSelected('게시판 관리')} to={`/study/${studyId}/board/manage`}>
                    게시판 관리{IsSelected === '게시판 관리' ? <UnderLine variants={UnderLineVariants} initial="hidden" animate="visible" /> : null }
                    </Category>}
            </CategoryWrapper>
            <Outlet />
        </>
